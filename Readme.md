...
# RL4T1D: Reinforcement Learning for Automating Treatment in Type 1 Diabetes.
...
* Clone the repository: `git clone git@github.com:RL4H/RL4T1D.git`.
* Go to the project folder (RL4T1D): `cd RL4T1D`
* Install the required Python libraries `requirements.txt`.
* Create an environment file `.env` at the root of the project folder with `MAIN_PATH=path-to-this-project` (`echo "MAIN_PATH=$(pwd)">.env`).
...
* You can also install from the source (Recommended to install using `pip install -e .`, the simglucose 0.2.2 source code is available in the environments folder). The simulation environment and scenarios used in this project are extended from the original environment.

### Quick Start - Running RL algorithms
...
### Quick Start - Running clinical treatment algorithms

Running a **clinical treatment** algorithm: **Basal-Bolus (BB)**. The parameters if the BB treatment can be updated via `clinical_treatment.yaml` file. More info: [Paper](https://ieeexplore.ieee.org/abstract/document/9871054)  .
```
cd experiments
...
RoadMap and Notes
--
* Now you can use mlflow to organise: <experiments, experiment_runs (e.g., for hyperparams, new ideas)>.
* We have legacy custom visualisation scripts <jupyter notebooks> used to debug experiments while running - integrate with new flow
* Think/Complete logging. Duplicated with mlflow logs and legacy custom logging patterns
  * validation results.
* Write test cases
* Add read docs like GluCoEnv.
* Improve handling offpolicy vs onpolicy workers.
