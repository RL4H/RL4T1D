...
OBJECT_SAVE_FILE = SIM_DATA_PATH + "/object_save/data_dictionary.pkl"
...
def import_raw_files(file_dest_folder=SIM_DATA_PATH + "/object_save/", file_name_start="data_dictionary_", file_name_end="_data"):
...
if __name__ == "__main__":
    ...
    if SINGLE_INDIVIDUAL_FILES: #import data of each individuals seperately (useful if you have limited RAM on your computer)
        import_raw_files()
    else: #read all data as single object and saves it as such

        start_time = datetime.now() #start the write timer
        file_dest = SIM_DATA_PATH + "/object_save/data_dictionary_whole.pkl"
        
        data = import_all_data(SIM_DATA_PATH, show_progress=True) #import data from files
        print("\nSuccesfully imported.")
